...
export class ListUsersComponent implements OnInit {
    ...
    let sub = this.openDeleteDialog(user).subscribe(
      (delet) => {
        if (delet?.emailIsCorrect) {
          console.log(user.id);
          let sub2 = this._userService.deleteUser(user.id).subscribe(
            (del) => {
  ...
  alterUser(user) {
    let sub = this.openEditDialog(user).subscribe(
      (alter) => {
        if (alter?.alterou) {
          let sub2 = this._userService.updateUser(alter.user).subscribe(
            (userApi) => {
