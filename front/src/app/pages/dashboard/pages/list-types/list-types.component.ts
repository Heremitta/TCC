...
export class ListTypesComponent {
  ...
  deleteUse(user: User) {
    let sub = this.openDeleteDialog(user).subscribe(
      (delet) => {
        if (delet?.choice) {
          console.log(delet);
          let sub2 = this._userService.deleteUserTypes(user.id).subscribe(
            (del) => {
  ...
  alterUser(type) {
    let sub = this.openEditDialog(type).subscribe(
      (alter) => {
        if (alter?.alterou) {
          let sub2 = this._userService.updateUserTypes(alter.type).subscribe(
            (userApi) => {
